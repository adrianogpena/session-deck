...
export async function activate(context: vscode.ExtensionContext) {
  ...
  // Before the tree first renders, so migrated names show from the start.
  try {
    await state.migrateLegacyState();
  } catch (err) {
    outputChannel.appendLine(`Could not copy session names/archive flags to ~/.session-deck: ${String(err)}`);
  }
